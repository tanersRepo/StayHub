...
/**
 * POST multipart/form-data: file, propertyId, durationSec (videos, client-measured).
...
export async function POST(req: Request) {
  ...
  const existing = await db.propertyMedia.count({ where: { propertyId, kind } });
  const limit = isImage ? MAX_IMAGES_PER_PROPERTY : MAX_VIDEOS_PER_PROPERTY;
  if (existing >= limit) {
    return NextResponse.json({ error: `You can upload at most ${limit} ${isImage ? "photos" : "videos"}` }, { status: 409 });
  }
